...
class EnsureUITester {
  constructor(options = {}) {
    ...
    this.concurrency = parseInt(options.concurrency || process.env.CONCURRENCY || 4);
  ...
  async findPageByRoute(targetRoute) {
    const pages = await this.findEnsureUIPages();
    return pages.find(page => page.route === targetRoute || page.route === targetRoute.replace(/\/$/, ''));
  }
...
IMPORTANT: If the input is empty, meaningless, or contains no actual test expectations (just keywords like "ensureUI", "test", whitespace), return empty expectations array: {"expectations": [], "urlParams": {}}
...
The generated code should be functional and demonstrate the intended behavior while making the placeholder logic clear and easily modifiable.Use await expect() for assertions.`
  ...
  async runTestsWithSemaphore(pages) {
    let runningTasks = 0;
    let completedTasks = 0;
    const results = new Array(pages.length);
    const completedTests = new Array(pages.length).fill(false);
    let nextOutputIndex = 0;

    const runTest = async (page, index) => {
      const logs = [];
    ...
    const outputTestResult = (testResult) => {
      const { result, logs } = testResult;
      ...
      console.log(`Ensure: ${result.rawExpectations}`);
      console.log(`Expectations: ${result.expectations.length}`);
    ...
    const processOutputQueue = () => {
      while (nextOutputIndex < pages.length && completedTests[nextOutputIndex]) {
        outputTestResult(results[nextOutputIndex]);
        nextOutputIndex++;
      }
    };
    ...
    return new Promise((resolve) => {
      ...
      const processNext = () => {
        if (pageIndex < pages.length && runningTasks < this.concurrency) {
          const page = pages[pageIndex];
          const currentIndex = pageIndex;
          pageIndex++;
          runningTasks++;

          runTest(page, currentIndex).then((testResult) => {
            results[currentIndex] = testResult;
            completedTests[currentIndex] = true;
            completedTasks++;
            runningTasks--;
            
            processOutputQueue();

            if (completedTasks === pages.length) {
              resolve(results);
            } else {
              processNext();
            }
  ...
  async runAllTests() {
    ...
    console.log(`Running with concurrency: ${this.concurrency}`);
    ...
    await this.runTestsWithSemaphore(pages);
